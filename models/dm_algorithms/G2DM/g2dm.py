...
class G2DM(DefaultModel):
    ...
    def setup_path(self):
        source_dataset = self.datasets['train']
        validation_dataset = self.datasets['val']
        target_dataset = self.datasets['target']

        self.source_loader = torch.utils.data.DataLoader(source_dataset, batch_size=self.flags.batch_size, shuffle=True,
                                                         num_workers=self.flags.nb_workers,
                                                         pin_memory=True)

        self.test_source_loader = torch.utils.data.DataLoader(validation_dataset, batch_size=self.flags.batch_size, shuffle=True,
                                                              num_workers=self.flags.nb_workers, pin_memory=True)

        self.target_loader = torch.utils.data.DataLoader(target_dataset, batch_size=self.flags.batch_size, shuffle=True,
                                                         num_workers=self.flags.nb_workers, pin_memory=True)
