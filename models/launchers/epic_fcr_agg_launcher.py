...
class Epi_FCR_AGG_Launcher(DefaultLauncher):
    ...
    def __init__(self, algorithm_dict, algorithm_name, flags, hparams, input_shape, datasets, class_idx,
        ...
        super(Epi_FCR_AGG_Launcher, self).__init__()
    ...
    def train_workflow(self):
        ...
        while self.current_epoch < flags.n_epochs:
            ...
            while again:
                ...
                domains = Variable(domains, requires_grad=False).cuda()
    ...
    def test_workflow(self, domain_key, loader):
        ...
        test_features_preds = None
        test_feature_truth = None
        test_probs = None
        ...
        with torch.no_grad():
            for x, y in loader:
                ...
                predictions = logits.cpu().data.numpy()
                probabilities = F.softmax(logits, dim=-1)

                if test_features_preds is None:
                    test_features_preds = []
                    test_probs = []
                    test_feature_truth = []

                test_features_preds.extend(predictions.tolist())
                test_feature_truth.extend(labels.tolist())
                test_probs.extend(probabilities.tolist())
